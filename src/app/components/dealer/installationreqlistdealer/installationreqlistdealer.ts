import { NgClass, NgFor } from '@angular/common';
import { HttpClient } from '@angular/common/http';
import { Component, inject } from '@angular/core';
import { FormsModule } from '@angular/forms';
import { Router } from '@angular/router';

@Component({
  imports: [FormsModule, NgClass, NgFor],
  selector: 'app-installationreqlistdealer',
  styleUrl: './installationreqlistdealer.css',
  templateUrl: './installationreqlistdealer.html',
})
export class Installationreqlistdealer {

  searchText = '';
  selectedStatus = '';
  selectedChargerType = '';

  router = inject(Router);

installations = [
  {
    installationNo: 'INSR-D1223232',
    customer: 'ABC Enterprises',
    chargerType: 'AC Charger',
    siteAddress: '123 MG Road',
    city: 'Pune',
    preferredDate: '12-Sep-2026',
    status: 'Pending'
  },
  {
    installationNo: 'INSR-D1223233',
    customer: 'XYZ Industries',
    chargerType: 'DC Charger',
    siteAddress: '45 Andheri East',
    city: 'Mumbai',
    preferredDate: '15-Sep-2026',
    status: 'In Progress'
  },
  {
    installationNo: 'INSR-D1223234',
    customer: 'PQR Solutions',
    chargerType: 'AC Charger',
    siteAddress: '78 Sector 18',
    city: 'Delhi',
    preferredDate: '18-Sep-2026',
    status: 'Completed'
  },
  {
    installationNo: 'INSR-D1223235',
    customer: 'Green Mobility',
    chargerType: 'Fast Charger',
    siteAddress: '22 Whitefield Road',
    city: 'Bangalore',
    preferredDate: '20-Sep-2026',
    status: 'Pending'
  }
];

applyFilters(): void {
  // Add API/filter logic here
}

resetFilters(): void {
  this.searchText = '';
  this.selectedStatus = '';
  this.selectedChargerType = '';
}

newInstallationRequest(): void {
  // Navigate to new installation page
  this.router.navigate(['/installation-request-dealer'])
}

viewInstallation(installation: any): void {
  // Navigate to installation details
}

}
